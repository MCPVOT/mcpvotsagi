...
# Web Service Interface for DGM Trading Algorithms (Legacy)
# =========================================================

class DGMTradingServiceLegacy:
    """Web service for DGM Trading Algorithms (Legacy)"""

    def __init__(self, port: int = 8005):
        self.port = port
        self.app = web.Application()

        # Initialize trading components with default strategy
        default_strategy = TradingStrategy()  # Use default parameters
        self.dgm = DynamicGodelMachine(initial_strategy=default_strategy)
        self.rl_trader = ReinforcementLearningTrader()
        self.engine = UnifiedTradingAlgorithmEngine()
    ...
    def setup_routes(self):
        ...
        self.app.router.add_post('/trade', self.execute_trade)
        self.app.router.add_post('/optimize', self.optimize_strategy)
    ...
    async def health_check(self, request):
        ...
        return web.json_response({
            'status': 'healthy',
            'service': 'DGM Trading Algorithms (Legacy)',
            'timestamp': datetime.now().isoformat(),
            'version': '1.0'
        })
    ...
    async def get_status(self, request):
        ...
        return web.json_response({
            'dgm_active': True,
            'rl_trader_active': True,
            'engine_active': True,
            'current_utility': self.dgm.current_utility if hasattr(self.dgm, 'current_utility') else 0.0,
            'timestamp': datetime.now().isoformat()
        })
    ...
    async def optimize_strategy(self, request):
    ...
    async def start_service(self):
        """Start the DGM Trading Legacy web service"""
        logger = logging.getLogger("DGMTradingLegacy")
        logger.info(f"🚀 DGM Trading Legacy Service starting on port {self.port}")
        ...
        logger.info(f"✅ DGM Trading Legacy Service running on http://localhost:{self.port}")

async def main():
    """Main entry point for DGM Trading Legacy Service"""
    service = DGMTradingServiceLegacy(port=8005)

    try:
        await service.start_service()

        print("✅ DGM Trading Algorithms (Legacy) Service is running!")
        print("📊 Available endpoints:")
        print("   - GET  /health     - Health check")
        print("   - GET  /status     - Trading system status")
        print("   - POST /trade      - Execute trade")
        print("   - POST /optimize   - Optimize strategy")
        ...
        print("\n🛑 Shutting down DGM Trading Legacy Service...")
